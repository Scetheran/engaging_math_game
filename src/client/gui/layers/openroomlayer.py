...
class OpenRoomLayer(GUILayer):
    def __init__(self):
        ...
        self._roomIDRect = pygame.Rect(self._gameConfig.screenCenterX - 4 * self._gameConfig.tileSize,
                                       self._gameConfig.screenCenterY - (3 * self._gameConfig.tileSize) // 2,
                                       8 * self._gameConfig.tileSize,
                                       self._gameConfig.tileSize)

        self._waitingOppRect = pygame.Rect(self._gameConfig.screenCenterX - 4 * self._gameConfig.tileSize,
    ...
    def _handleKeyPressedEvent(self, event):
        ...
        if (event.key == pygame.K_RETURN and self._goToLobbySelected) or event.key == pygame.K_ESCAPE:
    ...
    def _onRender(self, screen):
        ...
        pygame.draw.rect(screen, self._gameConfig.tileRectBorderColor, self._goToLobbyRect, self._gameConfig.tileBorderWidth, self._gameConfig.tileBorderRadius)
        screen.blit(self._goToLobbyRectText, self._goToLobbyRectText.get_rect(center=self._goToLobbyRect.center))
        if self._goToLobbySelected:
            pygame.draw.rect(screen, self._gameConfig.tileSelectorRectBorderColor, self._goToLobbyInnerRect, self._gameConfig.tileSelectorOffsetBorderWidth, self._gameConfig.tileBorderRadius)

        if self.isAwaitingSwitchOff():
            roomIDPlaceholderError = self._gameConfig.tileFont.render(self._errorMsg, True, self._gameConfig.tileRectBorderColor)
            screen.blit(roomIDPlaceholderError, roomIDPlaceholderError.get_rect(center=self._roomIDRect.center))
            return

        if self._roomID is not None:
            roomIDText = self._gameConfig.tileFont.render(f"Room code:  {self._roomID}", True, self._gameConfig.tileRectBorderColor)
            screen.blit(roomIDText, roomIDText.get_rect(center=self._roomIDRect.center))
            waitingOppText = self._gameConfig.tileFont.render("Waiting for opponent...", True, self._gameConfig.tileRectBorderColor)
            screen.blit(waitingOppText, waitingOppText.get_rect(center=self._waitingOppRect.center))
            return

        roomIDPlaceholder = self._gameConfig.tileFont.render(self._roomID, True, self._gameConfig.tileRectBorderColor)
        screen.blit(roomIDPlaceholder, roomIDPlaceholder.get_rect(center=self._roomIDRect.center))
