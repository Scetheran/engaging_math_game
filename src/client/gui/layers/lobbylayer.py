...
class LobbyLayer(GUILayer):
    def __init__(self):
        ...
        self._openRoomRect = pygame.Rect(
        ...
        self._openRoomRectText = self._gameConfig.tileFont.render(
            "Create new room", True, self._gameConfig.tileRectBorderColor
        )

        self._joinRoomRect = pygame.Rect(
            self._gameConfig.screenCenterX - 4 * self._gameConfig.tileSize,
            self._gameConfig.screenCenterY - self._gameConfig.tileSize,
            8 * self._gameConfig.tileSize,
            self._gameConfig.tileSize,
        )
        self._joinRoomRectText = self._gameConfig.tileFont.render(
            "Join Existing Room", True, self._gameConfig.tileRectBorderColor
        )

        self._quitGameRect = pygame.Rect(
            self._gameConfig.screenCenterX - 4 * self._gameConfig.tileSize,
            self._gameConfig.screenCenterY + self._gameConfig.tileSize // 2,
            8 * self._gameConfig.tileSize,
            self._gameConfig.tileSize,
        )
        self._quitGameRectText = self._gameConfig.tileFont.render(
            "Quit Game", True, self._gameConfig.tileRectBorderColor
        )
        self._rects = [
            (self._openRoomRect, self._openRoomRectText),
            (self._joinRoomRect, self._joinRoomRectText),
            (self._quitGameRect, self._quitGameRectText),
        ]
        ...
        self._events = [
            ...
            eventlist.LOBBYGUILAYER_QUITGAME_ID,
        ]
    ...
    def _subscribeToInternalEvents(self):
        return [
            eventlist.OPENROOMGUILAYER_TRANSLOBBY_ID,
            eventlist.JOINROOMGUILAYER_TRANSLOBBY_ID,
        ]
    ...
    def handleInternalEvent(self, event):
        ...
        if event.id == eventlist.JOINROOMGUILAYER_TRANSLOBBY_ID:
            self.switchOn()
