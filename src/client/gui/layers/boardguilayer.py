    ...
    def onUpdate(self):
        ...
        for column in range(8):
            for row in range(8):
                ...
                if column == availableColumn or row == availableRow:
                    # tileBorderColor = self._gameConfig["TILE_AVAILABLE_COLOR"]
                    tileBorderColor = (250, 40, 60)
                ...
                if column == self._selectorColumn and row == self._selectorRow:
                    ...
                    self._selectorOuterRect = (r, tileBorderColor)
                else:
                    self._rects.append((r, tileBorderColor))

    def _onRender(self, screen):
        screen.fill(self._gameConfig.backgroundColor)
        for (r, color) in self._rects:
            pygame.draw.rect(
                screen,
                color,
                r,
            ...
            text = self._gameConfig.tileFont.render(
                "16", True, self._gameConfig.tileRectBorderColor
            )
