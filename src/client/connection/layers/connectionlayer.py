...
class ConnectionLayer(GameAppLayer):
    ...
    def _handleSwitchOn(self):
        self._client = gameclient.GameClient(
            self._serverAddress, self._serverPort, self._clientType, self._pollrate
        )
    ...
    def _subscribeToInternalEvents(self):
        return [
            eventlist.BOARDGUILAYER_MOVEMADE_ID,
    ...
    def handleInternalEvent(self, event):
        if event.id == eventlist.BOARDGUILAYER_MOVEMADE_ID and self.isSwitchedOn():
            ...
            self._clientType = gameclient.GameClientType(
                gameclient.GameClientType.CREATE_ROOM
            )
            self.switchOn()
        elif event.id == eventlist.JOINROOMGUILAYER_JOINROOM_ID:
            self._clientType = gameclient.GameClientType(
                gameclient.GameClientType.JOIN_ROOM, event.data
            )
    ...
    def onUpdate(self):
        ...
        if not self._roomCreated:
            ...
            if roomCode is not None:
                self.pushInternalEvent(
                    eventlist.CONNECTIONLAYER_ROOMCREATED_ID, roomCode
                )
                self._roomCreated = True
            else:
                return
        ...
        if self._gamedata is not None:
            if self._gamedata.status == gamedata.GameStatus.OVER:
                if (
                    self._gamedata.playerData.ownScore
                    == self._gamedata.playerData.enemyScore
                ):
                    self.pushInternalEvent(eventlist.CONNECTIONLAYER_GAMEOVER_DRAW)
                if (
                    self._gamedata.playerData.ownScore
                    > self._gamedata.playerData.enemyScore
                ):
                    self.pushInternalEvent(eventlist.CONNECTIONLAYER_GAMEOVER_WON)
                if (
                    self._gamedata.playerData.ownScore
                    < self._gamedata.playerData.enemyScore
                ):
                    self.pushInternalEvent(eventlist.CONNECTIONLAYER_GAMEOVER_LOST)
                self.switchOff()
                return

            self.pushInternalEvent(
                eventlist.CONNECTIONLAYER_DATAUPDATED_ID, self._gamedata
            )
